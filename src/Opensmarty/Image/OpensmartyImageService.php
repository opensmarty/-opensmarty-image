...
class OpensmartyImageService
{
    ...
    {
        return config("opensmarty-image.$key", $default);
    }
    ...
    public function storeImageFromPath($path_with_name, $file_extension = null, $isAllowGIF = false, $isStorePNG = false)
    {
        ...
        if ($isExists) {
            ...
            $opensmarty_image_hash = OpensmartyImageHash::where(['file_sha1' => $file_sha1])->first();
            $isExists = $isSimilarExists = $opensmarty_image_hash ? true : false;
        }
        ...
        $opensmarty_image = null;
        if ($isExists) {
            $opensmarty_image = $this->saveImageInfo($isSimilarExists, $opensmarty_image_hash,
    ...
    public function storeImageFromURLEncodedImageData($originImageEncodedData, $isAllowGIF = false)
    {
        ...
        $is_gif = OpensmartyImageService::isDataURLEncodedImageGif($originImageEncodedData);
        $origin_image_data = OpensmartyImageService::convertDataURLEncodedToImageData($originImageEncodedData);
    ...
    {
        return OpensmartyImage::where('image_name', $image_name)->firstOrFail();
    }
    ...
    public function showImage($template_name, $image_name, array $image_templates = [], $storage_path = null)
    {
        ...
        if (!isset($image_templates[$template_name])) {
            throw new OpensmartyImageException('Invalid template name supplied.');
        }
     ...
     * @return OpensmartyImage
     */
    private function saveImageInfo($isSimilarExists, $opensmarty_image_hash, $final_file_name, $is_allowed_animated_gif, $final_file_size, $image, $exif, $file_sha1, $final_file_sha1)
    {
        // check has opensmarty image
        if ($isSimilarExists) {
            $opensmarty_image = $opensmarty_image_hash->image;
        } else {
            $opensmarty_image = OpensmartyImage::where('image_name', $final_file_name)->first();
        }
        ...
        if (!$opensmarty_image) {
            ...
            // create
            $opensmarty_image = OpensmartyImage::firstOrCreate([
            ...
            // add to image hash
            OpensmartyImageHash::firstOrCreate([
                'opensmarty_image_id' => $opensmarty_image->opensmarty_image_id,
                'file_sha1' => $file_sha1,
            ]);
    ...
    /**
     * @param integer $opensmarty_image_id
     * @return \Intervention\Image\Image
     */
    public static function getImageFromOpensmartyImageId($opensmarty_image_id)
    {
        $opensmarty_image = OpensmartyImage::find($opensmarty_image_id);
        if (is_null($opensmarty_image)) {
            throw new OpensmartyImageException('Opensmarty image record not found!');
        }
        $imageManager = new ImageManager(config('image'));
        $background_image = $imageManager->make($opensmarty_image->getImagePath());
